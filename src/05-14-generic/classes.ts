...
class UniqueFilter<T> implements I.Filter<T> {
  unique(arr: Array<T>): Array<T> {
...
class Manager extends Person implements I.Manager {
  ...
  constructor(_name: string, _department: number) {
    super(_name);
...
export { UniqueFilter, Person, Employee, Manager };
